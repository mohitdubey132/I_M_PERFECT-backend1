...
@api_view(['GET'])
def user_AllQues_Ans(request, uid):
    user_id = get_user_id_from_token(request)
    if user_id is not None:
        try:
            questions = Question.objects.filter(U_id=uid)
            count_of_ques = questions.count()
            serializer1 = QuestionSerializer(questions, many=True)
            answers = Answers.objects.filter(U_id=uid)
            count_of_ans = answers.count()
            serializer2 = AnswersSerializer(answers, many=True)

            data = {
                'count_of_ques' :  count_of_ques,
                'ques_data': serializer1.data ,
                'count_of_ans' :  count_of_ans,
                'ans_data': serializer2.data ,
                'message' : 'answers for question',
                'api_status' : 200
                }
            
            return Response({'data':data, 'api_status':True, 'message':'Success'}, status=status.HTTP_200_OK)
...
def update_question(request, question_id):
    ...
    if user_id is not None:
            ...
            if user_id == question.U_id:
                deserializer = QuestionSerializer(question, data=request.data, partial=True)

                if deserializer.is_valid():
                    deserializer.save()
                    return Response({'data':deserializer.data, 'api_status':True, 'message':'Success'}, status=status.HTTP_200_OK)
                else:
                    return Response(deserializer.errors, status=status.HTTP_400_BAD_REQUEST)
            else:
                return Response({"error": "You don't have permission to update this question"}, status=status.HTTP_403_FORBIDDEN)
